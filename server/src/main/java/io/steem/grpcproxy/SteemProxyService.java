...
public class SteemProxyService extends SteemServiceGrpc.SteemServiceImplBase {
  ...
  public void login(LoginRequest request, StreamObserver<LoginResponse> responseObserver) {
    try {
      ...
      boolean succeeded = LoginApi.login(communicationHandler, accountName, request.getPassword());
  ...
  public void getRecentTrades(
      GetRecentTradesRequest request, StreamObserver<GetRecentTradesResponse> responseObserver) {
    try {
      ...
      List<MarketTrade> trades =
          steemjTrades
              .stream()
              .map(MarketTradeConverter::toGrpc)
